...
# WeasyPrint 延迟导入，避免在模块加载时出错
WEASYPRINT_AVAILABLE = None  # None = 尚未检查
_weasyprint_module = None


def _ensure_weasyprint():
    """确保 WeasyPrint 可用，只在第一次调用时检查"""
    global WEASYPRINT_AVAILABLE, _weasyprint_module
    if WEASYPRINT_AVAILABLE is not None:
        return WEASYPRINT_AVAILABLE
    
    try:
        from weasyprint import HTML, CSS
        _weasyprint_module = (HTML, CSS)
        WEASYPRINT_AVAILABLE = True
        return True
    except Exception as e:
        WEASYPRINT_AVAILABLE = False
        print(f"[INFO] WeasyPrint 不可用: {e}")
        return False
...
class PDFService:
    ...
    def __init__(self):
        ...
        self.available = _ensure_weasyprint()
    ...
    def generate_pdf(self, itinerary_data: Dict, itinerary_title: str,
        ...
        try:
            ...
            if _weasyprint_module:
                HTML, CSS = _weasyprint_module
