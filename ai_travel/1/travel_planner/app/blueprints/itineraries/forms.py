...
class ItineraryStep1Form(FlaskForm):
    ...
    use_custom_city = BooleanField('使用自定义城市（城市不在列表中？）')
    custom_city = StringField('自定义城市名称', validators=[Optional()])
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 动态加载目的地选项
        destinations = Destination.query.order_by(Destination.view_count.desc()).all()
        self.destination_id.choices = [(0, '-- 选择一个目的地 --')] + [(d.id, f"{d.name} - {d.city}") for d in destinations]
